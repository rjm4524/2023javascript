...
<html>
	<head>
		...
		<style>
			...
			span:after {content:","; color:blue}
		</style>
	</head>
	<body>
	<h2>섹션 : <%=session.getAttribute("session_nicname")%></h2>
	<h2>취미 : <%=Arrays.toString(request.getParameterValues("hobby"))%></h2>
	...
	<h2>결과값1 :<%=Integer.parseInt(request.getParameter("num1"))+Integer.parseInt(request.getParameter("num2")) %></h2>
	<h2>결과값2 :<%=request.getParameter("num1")+request.getParameter("num2") %></h2>
	<hr>
	<h2>섹션1 : ${sessionScope.session_nicname}</h2>
	<h2>섹션2 : ${session_nicname}</h2>
	<h2>취미 : ${paramValues.hobby}</h2>
